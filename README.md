...
说明：pod中不包含gif表情的UI模块，可点击[下载](http://7xl6jm.com2.z0.glb.qiniucdn.com/release/ios-sdk/BQMM_GIF.zip)，解压后，手动导入`BQMM_GIF`
...
`NTESChatroomEmojiContentView.h`、`NIMSessionEmojiContentView`中的`UIImageView`相应的改成了`MMImageView`
...
2. `NIMTextContentConfig`中的`cellContent`用于判断消息是普通的文字消息还是表情消息。
```objectivec
